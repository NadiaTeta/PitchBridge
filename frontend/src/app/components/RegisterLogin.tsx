...
export function RegisterLogin() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        // We capture the user object returned from the login function
        const user = await login(formData.email, formData.password);
        
        // SMART REDIRECT: Logic to send Admins to their specific dashboard
        ...
        await register({ name: formData.fullName, email: formData.email, password: formData.password, role });
      ...
      <div className="p-6 max-w-md mx-auto">
        <div className="bg-white rounded-xl shadow-sm p-6 mb-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {/* Role Display - Only visible during registration */}
            {!isLogin && role && (
              <div>
                <label className="block text-sm mb-2">Account Type</label>
                <div className="px-4 py-3 bg-blue-50 border border-blue-200 rounded-xl">
                  <p className="capitalize font-medium text-blue-900">{role}</p>
                </div>
              </div>
            )}
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-400 text-white rounded-xl transition-colors font-semibold"
            >
